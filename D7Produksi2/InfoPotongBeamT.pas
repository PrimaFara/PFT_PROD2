...
type
  TInfoPotongBeamTFrm = class(TForm)
    ...
    QBrowse3TANGGAL_MUTASI: TDateTimeField;
    QBrowse3ID_RAB: TFloatField;
    QBrowse3NO_ORDER: TStringField;
    QBrowse3VERSI: TStringField;
    QBrowse3KP: TStringField;
    QBrowse3NO_DESAIN: TFloatField;
    QBrowse3JML_P_ORDER: TFloatField;
    QBrowse3JML_K_ORDER: TFloatField;
    QBrowse3NO_BEAM: TStringField;
    QBrowse3KONSTRUKSI: TStringField;
    QBrowse3CORAK: TStringField;
    QBrowse3JML_POTONG: TFloatField;
    QBrowse3JML_KODI: TFloatField;
    QBrowse3PROSES: TStringField;
    QBrowse3LOKASI: TStringField;
    QBrowse3PROSES_TUJUAN: TStringField;
    QBrowse3LOKASI_TUJUAN: TStringField;
    QBrowse3HASIL: TFloatField;
...
var
  InfoPotongBeamTFrm: TInfoPotongBeamTFrm;
...
procedure TInfoPotongBeamTFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  InfoPotongBeamTFrm:=Nil;
end;

procedure TInfoPotongBeamTFrm.TabSheet2Show(Sender: TObject);
//var
  //vrgTanggal : String;
begin
...
procedure TInfoPotongBeamTFrm.vTglAwalChange(Sender: TObject);
begin
  vTglAkhir.DateTime:=EndOfTheMonth(vTglAwal.Date);
end;

procedure TInfoPotongBeamTFrm.BtnExportClick(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.FormShow(Sender: TObject);
begin
  vTglAwal.Date:=Date;
end;

procedure TInfoPotongBeamTFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TInfoPotongBeamTFrm.vOperandClick(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
  vrgTanggal : String;
...
procedure TInfoPotongBeamTFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TInfoPotongBeamTFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRencord.Caption:=IntToStr(QBrowse.RecNo)+' of '+IntToStr(QBrowse.RecordCount);
end;

procedure TInfoPotongBeamTFrm.BitBtn2Click(Sender: TObject);
var
  i : word;
  vpertama2 : boolean;
begin
...
procedure TInfoPotongBeamTFrm.TabSheet1Show(Sender: TObject);
begin
QBrowse2.Close;
QBrowse2.Open;
end;

procedure TInfoPotongBeamTFrm.BitBtn10Click(Sender: TObject);
...
begin
    ...
    QAmbil_Data.Execute;
    vfilter:=' where (tanggal>=to_date('''+FormatDateTime('dd/mm/yyyy',VTglAwal1.Date)+''',''dd/mm/yyyy'') and tanggal<=(to_date('''+FormatDateTime('dd/mm/yyyy',VTglAkhir1.Date)+''',''dd/mm/yyyy'')+1-1/86400))';
    ...
    vfilter:=' where (tanggal_mutasi>=to_date('''+FormatDateTime('dd/mm/yyyy',VTglAwal1.Date)+''',''dd/mm/yyyy'') and tanggal_mutasi<=(to_date('''+FormatDateTime('dd/mm/yyyy',VTglAkhir1.Date)+''',''dd/mm/yyyy'')+1-1/86400))';
    QBrowse3.SetVariable('myparam',vfilter);
    QBrowse3.Open;
    end;
...
procedure TInfoPotongBeamTFrm.VTglAwal1Change(Sender: TObject);
begin
  vTglAkhir1.DateTime:=EndOfTheMonth(vTglAwal1.Date);
end;

procedure TInfoPotongBeamTFrm.vTglAkhir1Change(Sender: TObject);
begin
...
procedure TInfoPotongBeamTFrm.QBrowse3AfterScroll(DataSet: TDataSet);
begin
  Label5.Caption:=IntToStr(QBrowse3.RecNo)+' of '+IntToStr(QBrowse3.RecordCount);
end;

procedure TInfoPotongBeamTFrm.BitBtn6Click(Sender: TObject);
var
  i : word;
  vpertama3 : boolean;
  vrgTanggal3 : String;
begin
  ...
  begin
    case rgTanggal1.ItemIndex of
    0 : vrgTanggal3:='TANGGAL';
    1 : vrgTanggal3:='TANGGAL_MUTASI';
    end;
    vfilter:=' where ('+vrgTanggal3+'>=to_date('''+FormatDateTime('dd/mm/yyyy',VTglAwal1.Date)+''',''dd/mm/yyyy'') and '+vrgTanggal3+'<=(to_date('''+FormatDateTime('dd/mm/yyyy',VTglAkhir1.Date)+''',''dd/mm/yyyy'')+1-1/86400)) and (';
  end
  else
    vfilter:=' where (';
  ...
  //    for i:=0 to QBrowse.FieldCount-1 do
      begin
        if (QBrowse3.FieldByName(wwDBGrid3.Columns[i].FieldName).FieldKind=fkData) and (UpperCase(wwDBGrid3.Columns[i].FieldName)<>'TANGGAL') and (UpperCase(wwDBGrid3.Columns[i].FieldName)<>'TANGGAL_MUTASI') then
        begin
          if vpertama3 then
            begin
...
procedure TInfoPotongBeamTFrm.BitBtn7Click(Sender: TObject);
begin
  if QBrowse3.Active then
  begin
     DMFrm.SaveDialog1.DefaultExt:='XLK';
     DMFrm.SaveDialog1.Filter:='Excel files (*.XLK)|*.XLK';
     DMFrm.SaveDialog1.FileName:='INFO SISA POTONG BEAM';
     wwDBGrid2.ExportOptions.TitleName:='INFO SISA POTONG BEAM';
       if DMFrm.SaveDialog1.Execute then
       begin
         try
...
procedure TInfoPotongBeamTFrm.TabSheet3Show(Sender: TObject);
begin
//QBrowse3.Open;
end;

procedure TInfoPotongBeamTFrm.vTglAwal2Change(Sender: TObject);
begin
