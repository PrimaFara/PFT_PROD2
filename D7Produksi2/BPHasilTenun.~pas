...
type
  TBPHasilTenunFrm = class(TForm)
    ...
    QLookBeamSISA: TFloatField;
    QLookBeamJENIS_MESIN: TStringField;
    QBrowseJENIS_MESIN: TStringField;
    QBrowseE1: TFloatField;
    QBrowseE2: TFloatField;
    QBrowseF1: TFloatField;
    QBrowseF2: TFloatField;
    QBrowseG1: TFloatField;
    QBrowseG2: TFloatField;
    QTotalE1: TFloatField;
    QTotalE2: TFloatField;
    QTotalF1: TFloatField;
    QTotalF2: TFloatField;
    QTotalG1: TFloatField;
    QTotalG2: TFloatField;
...
var
  BPHasilTenunFrm: TBPHasilTenunFrm;

implementation

uses DM, HasilTenun, HasilTenunKain;

{$R *.dfm}

procedure TBPHasilTenunFrm.BtnFindClick(Sender: TObject);
begin
...
procedure TBPHasilTenunFrm.BtnOk2Click(Sender: TObject);
begin
...
procedure TBPHasilTenunFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
     DMFrm.SaveDialog1.DefaultExt:='XLK';
     DMFrm.SaveDialog1.Filter:='Excel files (*.XLK)|*.XLK';
     DMFrm.SaveDialog1.FileName:='HASIL TENUN';
     wwDBGrid2.ExportOptions.TitleName:='HASIL TENUN';
...
procedure TBPHasilTenunFrm.FormShow(Sender: TObject);
begin
  QBrowseNO_NOTA.AsString:= '1';
  QBrowse.Close;
  QBrowse.SetVariable('plokasi', HasilTenunFrm.QMasterSHIFT.AsString);
  QBrowse.Open;

end;

procedure TBPHasilTenunFrm.QBrowseNewRecord(DataSet: TDataSet);
begin
  QBrowseNO_REG.AsInteger:=HasilTenunKainFrm.QMasterNO_REG.AsInteger;
  if vnota<>'' then
...
procedure TBPHasilTenunFrm.QBrowseBeforeInsert(DataSet: TDataSet);
begin
  vnota:=QBrowseNO_NOTA.AsString;
  vlokasi:=QBrowseLOKASI.AsString;
end;

procedure TBPHasilTenunFrm.QBrowseBeforeQuery(Sender: TOracleDataSet);
begin
  QBrowse.SetVariable('no_reg',HasilTenunFrm.QMasterNO_REG.AsInteger);
...
procedure TBPHasilTenunFrm.QTotalBeforeQuery(Sender: TOracleDataSet);
begin
  QTotal.SetVariable('no_reg',HasilTenunFrm.QMasterNO_REG.AsInteger);

end;

procedure TBPHasilTenunFrm.wwDBGrid2UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
  QTotal.Open;
  ...
  wwDBGrid2.ColumnByName('E1').FooterValue:=FormatFloat('#,#',QTotalE1.AsFloat);
  wwDBGrid2.ColumnByName('E2').FooterValue:=FormatFloat('#,#',QTotalE2.AsFloat);
  wwDBGrid2.ColumnByName('F1').FooterValue:=FormatFloat('#,#',QTotalF1.AsFloat);
  wwDBGrid2.ColumnByName('F2').FooterValue:=FormatFloat('#,#',QTotalF2.AsFloat);
  wwDBGrid2.ColumnByName('G1').FooterValue:=FormatFloat('#,#',QTotalG1.AsFloat);
  wwDBGrid2.ColumnByName('G2').FooterValue:=FormatFloat('#,#',QTotalG2.AsFloat);
  wwDBGrid2.ColumnByName('TOTAL').FooterValue:=FormatFloat('#,#',QTotalTOTAL.AsFloat);
end;

procedure TBPHasilTenunFrm.QBrowseBeforePost(DataSet: TDataSet);
begin
...
procedure TBPHasilTenunFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  wwDBGrid2UpdateFooter(nil);
end;

procedure TBPHasilTenunFrm.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  QRLPeriode.Caption:=vTglAwal.Text+' dan '+vTglAkhir.Text;
end;

procedure TBPHasilTenunFrm.BitBtn2Click(Sender: TObject);
begin
  QuickRep1.Preview;
end;

procedure TBPHasilTenunFrm.wwCheckBox1Click(Sender: TObject);
begin
...
procedure TBPHasilTenunFrm.Edit1Change(Sender: TObject);
begin
...
procedure TBPHasilTenunFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String );
  var
  vorder: String;
begin
...
procedure TBPHasilTenunFrm.LookBeamEnter(Sender: TObject);
begin
  QLookBeam.Close;
...
procedure TBPHasilTenunFrm.LookBeamCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
if modified then
  begin
    QBrowseJENIS_MESIN.AsString:=QLookBeamJENIS_MESIN.AsString;
    QBrowseSISA.AsString:=QLookBeamSISA.AsString;
    QBrowseRESEP.AsString:=QLookBeamRESEP.AsString;
    QBrowseRM_LUSI.AsFloat:=QLookBeamRM_LUSI.AsFloat;
    QBrowseRM_PAKAN.AsFloat:=QLookBeamRM_PAKAN.AsFloat;
