...
var
  HasilTenun1Frm: THasilTenun1Frm;
...
procedure THasilTenun1Frm.FormClose(Sender: TObject;
...
procedure THasilTenun1Frm.TabSheet2Show(Sender: TObject);
begin
  ...
  vorder:=' group by no_order, konstruksi, corak, no_seri_beam';
  QBrowse.SetVariable('myparam',vfilter+vorder);
  QBrowse.Open;
end;

procedure THasilTenun1Frm.vTglAwalChange(Sender: TObject);
begin
  vTglAkhir.DateTime:=EndOfTheMonth(vTglAwal.Date);
end;

procedure THasilTenun1Frm.BtnExportClick(Sender: TObject);
begin
...
procedure THasilTenun1Frm.FormShow(Sender: TObject);
begin
  vTglAwal.Date:=Date;
end;

procedure THasilTenun1Frm.vTglAkhirChange(Sender: TObject);
begin
...
procedure THasilTenun1Frm.cbOtomatisClick(Sender: TObject);
begin
...
procedure THasilTenun1Frm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure THasilTenun1Frm.vOperandClick(Sender: TObject);
begin
...
procedure THasilTenun1Frm.cbTanggalClick(Sender: TObject);
begin
...
procedure THasilTenun1Frm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
  ...
  //    for i:=0 to QBrowse.FieldCount-1 do
    ...
    vfilter:=vfilter+')';
    if QBrowse.Active then
      vorder:=' group by no_order, konstruksi, corak, no_seri_beam order by '+wwDBGrid2.Columns[0].FieldName
      else
        vorder:=' group by no_order, konstruksi, corak, no_seri_beam order by no_reg';
...
procedure THasilTenun1Frm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  ...
  begin
     if (Sender as TwwDBGrid).DataSource.DataSet.FieldByName(AFieldName).FieldKind=fkData then
        begin
          ...
          ((Sender as TwwDBGrid).DataSource.DataSet as TOracleDataSet).SetVariable('myparam',vfilter+' group by no_order, konstruksi, corak, no_seri_beam order by '+(Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName+vorder);
...
procedure THasilTenun1Frm.QBrowseAfterScroll(DataSet: TDataSet);
begin
{azmi}
