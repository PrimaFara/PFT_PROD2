object RekapBenangFrm: TRekapBenangFrm
  Left = 254
  Top = 137
  Width = 1022
  Height = 594
  Caption = 'Rekap Benang'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PageControl1: TPageControl
    Left = 0
    Top = 0
    Width = 1006
    Height = 556
    ActivePage = TabSheet2
    Align = alClient
    TabOrder = 0
    OnEnter = PageControl1Enter
    object TabSheet2: TTabSheet
      Caption = 'Benang Lusi'
      ImageIndex = 1
      OnShow = TabSheet2Show
      object Panel2: TPanel
        Left = 0
        Top = 487
        Width = 998
        Height = 41
        Align = alBottom
        TabOrder = 0
        DesignSize = (
          998
          41)
        object LRencord: TLabel
          Left = 16
          Top = 8
          Width = 5
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -19
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn1: TBitBtn
          Left = 742
          Top = 8
          Width = 75
          Height = 25
          Anchors = [akTop, akRight]
          TabOrder = 0
          Kind = bkClose
        end
      end
      object wwDBGrid2: TwwDBGrid
        Left = 0
        Top = 52
        Width = 998
        Height = 435
        ControlType.Strings = (
          'ISPOST;CheckBox;1;0'
          'PILIH;CheckBox;1;0')
        Selected.Strings = (
          'PILIH'#9'1'#9'PILIH'#9'F'
          'TGL'#9'18'#9'TANGGAL'#9'F'
          'NO_DESAIN'#9'10'#9'NO DESAIN'#9'F'
          'NO_ORDER'#9'20'#9'NO RESEP'#9'F'
          'KD_KONSTRUKSI'#9'6'#9'KODE'#9'F'#9'KONSTRUKSI'
          'NAMA_KONSTRUKSI'#9'25'#9'NAMA'#9'F'#9'KONSTRUKSI'
          'KD_CORAK'#9'6'#9'KODE'#9'F'#9'CORAK'
          'NAMA_CORAK'#9'20'#9'NAMA'#9'F'#9'CORAK'
          'KD_ITEM'#9'15'#9'KODE'#9'F'#9'BAHAN'
          'NAMA_ITEM'#9'25'#9'NAMA'#9'F'#9'BAHAN'
          'RASIO'#9'10'#9'RASIO'#9'F'#9'BAHAN'
          'ARAH'#9'10'#9'ARAH'#9'F'
          'WARNA'#9'25'#9'WARNA'#9'F'
          'NHELAI'#9'10'#9'JML'#9'F'#9'HELAI'
          'THELAI'#9'10'#9'TOTAL'#9'F'#9'HELAI'
          'THELAI2'#9'10'#9'PAKAN'#9'F'#9'HELAI'
          'PHELAI'#9'10'#9'METER'#9'F'#9'PER HELAI'
          'KGHELAI'#9'10'#9'GRAM'#9'F'#9'PER HELAI'
          'PSARUNG'#9'10'#9'GRAM'#9'F'#9'PER PALEKAT'
          'KGSARUNG'#9'10'#9'KG'#9'F'#9'PER PALEKAT'
          'PKODI'#9'10'#9'METER'#9'F'#9'PER KODI'
          'KGKODI'#9'10'#9'KG'#9'F'#9'PER KODI'
          'KGWASTE'#9'10'#9'METER'#9'F'#9'WASTE KODI'
          'PWASTE'#9'10'#9'KG'#9'F'#9'WASTE KODI'
          'PNET'#9'10'#9'METER'#9'F'#9'NETTO KODI'
          'KGNET'#9'10'#9'KG'#9'F'#9'NETTO KODI')
        IniAttributes.Enabled = True
        IniAttributes.SaveToRegistry = True
        IniAttributes.FileName = 'PRODUKSI2'
        IniAttributes.Delimiter = ';;'
        ExportOptions.ExportType = wwgetSYLK
        ExportOptions.Options = [esoShowHeader, esoShowFooter, esoShowTitle, esoDblQuoteFields, esoBestColFit, esoShowRecordNo, esoShowAlternating]
        TitleColor = clBtnFace
        FixedCols = 0
        ShowHorzScrollBar = True
        EditControlOptions = [ecoSearchOwnerForm, ecoDisableDateTimePicker]
        Align = alClient
        Color = 16773589
        Ctl3D = True
        DataSource = dsQBrowse
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgShowFooter, dgTrailingEllipsis, dgDblClickColSizing]
        ParentCtl3D = False
        ParentFont = False
        RowHeightPercent = 125
        TabOrder = 1
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlue
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = [fsBold]
        TitleLines = 3
        TitleButtons = True
        UseTFields = False
        OnTitleButtonClick = wwDBGrid2TitleButtonClick
        OnDblClick = wwDBGrid2DblClick
      end
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 998
        Height = 52
        Align = alTop
        BevelInner = bvLowered
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 2
        object GroupBox1: TGroupBox
          Left = 2
          Top = 2
          Width = 287
          Height = 48
          Align = alLeft
          Caption = 'Filter Tanggal'
          TabOrder = 0
          TabStop = True
          object Label6: TLabel
            Left = 132
            Top = 22
            Width = 24
            Height = 13
            Caption = 'S/D'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object VTglAwal: TwwDBDateTimePicker
            Left = 8
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 0
            DisplayFormat = 'dd mmm yyyy'
            OnChange = vTglAwalChange
          end
          object vTglAkhir: TwwDBDateTimePicker
            Left = 160
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 1
            DisplayFormat = 'dd mmm yyyy'
            OnChange = vTglAkhirChange
          end
        end
        object Panel7: TPanel
          Left = 289
          Top = 2
          Width = 568
          Height = 48
          Align = alLeft
          BevelOuter = bvNone
          TabOrder = 1
          object vOperand: TLabel
            Left = 150
            Top = 20
            Width = 44
            Height = 24
            Cursor = crHandPoint
            Alignment = taCenter
            AutoSize = False
            Caption = 'LIKE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            OnClick = vOperandClick
          end
          object cbTanggal: TCheckBox
            Left = 200
            Top = 2
            Width = 129
            Height = 17
            Caption = 'Ikutkan Filter &Tanggal'
            Checked = True
            Enabled = False
            State = cbChecked
            TabOrder = 1
            OnClick = cbTanggalClick
          end
          object BitBtn3: TBitBtn
            Left = 328
            Top = 20
            Width = 75
            Height = 25
            Caption = 'Filte&r'
            Default = True
            ModalResult = 1
            TabOrder = 3
            OnClick = BitBtn3Click
            Glyph.Data = {
              
              5990555555555555577755555555555555555555555555555555}
            NumGlyphs = 2
          end
          object ECari: TEdit
            Left = 200
            Top = 23
            Width = 121
            Height = 19
            Ctl3D = False
            ParentCtl3D = False
            TabOrder = 2
          end
          object cbOtomatis: TCheckBox
            Left = 7
            Top = 5
            Width = 98
            Height = 17
            Caption = 'Cari &Otomatis'
            TabOrder = 0
            OnClick = cbOtomatisClick
          end
          object dbcField: TwwDBComboBox
            Left = 8
            Top = 23
            Width = 137
            Height = 19
            ShowButton = True
            Style = csDropDown
            MapList = False
            AllowClearKey = False
            Color = clYellow
            DropDownCount = 8
            ItemHeight = 0
            Items.Strings = (
              'SEMUA KOLOM')
            ItemIndex = 0
            Sorted = False
            TabOrder = 4
            UnboundDataType = wwDefault
            OnEnter = dbcFieldEnter
          end
          object BitBtn4: TBitBtn
            Left = 408
            Top = 20
            Width = 97
            Height = 25
            Caption = '&Export2Excel'
            TabOrder = 5
            OnClick = BitBtn4Click
            Glyph.Data = {
              
              0003737FFFFFFFFF7F7330099999999900333777777777777733}
            NumGlyphs = 2
          end
        end
      end
    end
    object TabSheet1: TTabSheet
      Caption = 'Benang Pakan'
      ImageIndex = 1
      OnShow = TabSheet1Show
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 998
        Height = 52
        Align = alTop
        BevelInner = bvLowered
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 0
        object GroupBox2: TGroupBox
          Left = 2
          Top = 2
          Width = 287
          Height = 48
          Align = alLeft
          Caption = 'Filter Tanggal'
          TabOrder = 0
          TabStop = True
          object Label1: TLabel
            Left = 132
            Top = 22
            Width = 24
            Height = 13
            Caption = 'S/D'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object VTglAwal2: TwwDBDateTimePicker
            Left = 8
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 0
            DisplayFormat = 'dd mmm yyyy'
            OnChange = VTglAwal2Change
          end
          object vTglAkhir2: TwwDBDateTimePicker
            Left = 160
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 1
            DisplayFormat = 'dd mmm yyyy'
            OnChange = vTglAkhir2Change
          end
        end
        object Panel4: TPanel
          Left = 289
          Top = 2
          Width = 568
          Height = 48
          Align = alLeft
          BevelOuter = bvNone
          TabOrder = 1
          object vOperand2: TLabel
            Left = 150
            Top = 20
            Width = 44
            Height = 24
            Cursor = crHandPoint
            Alignment = taCenter
            AutoSize = False
            Caption = 'LIKE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            OnClick = vOperandClick
          end
          object cbTanggal2: TCheckBox
            Left = 200
            Top = 2
            Width = 129
            Height = 17
            Caption = 'Ikutkan Filter &Tanggal'
            Checked = True
            Enabled = False
            State = cbChecked
            TabOrder = 1
            OnClick = cbTanggalClick
          end
          object BitBtn2: TBitBtn
            Left = 328
            Top = 20
            Width = 75
            Height = 25
            Caption = 'Filte&r'
            Default = True
            ModalResult = 1
            TabOrder = 3
            OnClick = BitBtn2Click
            Glyph.Data = {
              
              5990555555555555577755555555555555555555555555555555}
            NumGlyphs = 2
          end
          object ECari2: TEdit
            Left = 200
            Top = 23
            Width = 121
            Height = 19
            Ctl3D = False
            ParentCtl3D = False
            TabOrder = 2
          end
          object cbOtomatis2: TCheckBox
            Left = 7
            Top = 5
            Width = 98
            Height = 17
            Caption = 'Cari &Otomatis'
            TabOrder = 0
            OnClick = cbOtomatisClick
          end
          object dbcField2: TwwDBComboBox
            Left = 8
            Top = 23
            Width = 137
            Height = 19
            ShowButton = True
            Style = csDropDown
            MapList = False
            AllowClearKey = False
            Color = clYellow
            DropDownCount = 8
            ItemHeight = 0
            Items.Strings = (
              'SEMUA KOLOM')
            ItemIndex = 0
            Sorted = False
            TabOrder = 4
            UnboundDataType = wwDefault
            OnEnter = dbcField2Enter
          end
          object BitBtn5: TBitBtn
            Left = 408
            Top = 20
            Width = 97
            Height = 25
            Caption = '&Export2Excel'
            TabOrder = 5
            OnClick = BitBtn5Click
            Glyph.Data = {
              
              0003737FFFFFFFFF7F7330099999999900333777777777777733}
            NumGlyphs = 2
          end
        end
      end
      object Panel9: TPanel
        Left = 0
        Top = 487
        Width = 998
        Height = 41
        Align = alBottom
        TabOrder = 1
        DesignSize = (
          998
          41)
        object Label4: TLabel
          Left = 16
          Top = 8
          Width = 5
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -19
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn9: TBitBtn
          Left = 742
          Top = 8
          Width = 75
          Height = 25
          Anchors = [akTop, akRight]
          TabOrder = 0
          Kind = bkClose
        end
      end
      object wwDBGrid1: TwwDBGrid
        Left = 0
        Top = 52
        Width = 998
        Height = 435
        DisableThemes = True
        DisableThemesInTitle = True
        ControlType.Strings = (
          'ISPOST;CheckBox;1;0'
          'PILIH;CheckBox;1;0')
        Selected.Strings = (
          'PILIH'#9'1'#9'PILIH'#9'F'
          'TGL'#9'18'#9'TANGGAL'#9'F'
          'NO_DESAIN'#9'10'#9'NO DESAIN'#9'F'
          'NO_ORDER'#9'20'#9'NO RESEP'#9'F'
          'KD_KONSTRUKSI'#9'6'#9'KODE'#9'F'#9'KONSTRUKSI'
          'NAMA_KONSTRUKSI'#9'25'#9'NAMA'#9'F'#9'KONSTRUKSI'
          'KD_CORAK'#9'6'#9'KODE'#9'F'#9'CORAK'
          'NAMA_CORAK'#9'20'#9'NAMA'#9'F'#9'CORAK'
          'KD_ITEM'#9'15'#9'KODE'#9'F'#9'BAHAN'
          'NAMA_ITEM'#9'25'#9'NAMA'#9'F'#9'BAHAN'
          'RASIO'#9'10'#9'RASIO'#9'F'#9'BAHAN'
          'ARAH'#9'10'#9'ARAH'#9'F'
          'WARNA'#9'25'#9'WARNA'#9'F'
          'NHELAI'#9'10'#9'JML'#9'F'#9'HELAI'
          'THELAI'#9'10'#9'TOTAL'#9'F'#9'HELAI'
          'THELAI2'#9'10'#9'PAKAN'#9'F'#9'HELAI'
          'PHELAI'#9'10'#9'METER'#9'F'#9'PER HELAI'
          'KGHELAI'#9'10'#9'GRAM'#9'F'#9'PER HELAI'
          'PSARUNG'#9'10'#9'GRAM'#9'F'#9'PER PALEKAT'
          'KGSARUNG'#9'10'#9'KG'#9'F'#9'PER PALEKAT'
          'PKODI'#9'10'#9'METER'#9'F'#9'PER KODI'
          'KGKODI'#9'10'#9'KG'#9'F'#9'PER KODI'
          'KGWASTE'#9'10'#9'METER'#9'F'#9'WASTE KODI'
          'PWASTE'#9'10'#9'KG'#9'F'#9'WASTE KODI'
          'PNET'#9'10'#9'METER'#9'F'#9'NETTO KODI'
          'KGNET'#9'10'#9'KG'#9'F'#9'NETTO KODI')
        IniAttributes.Enabled = True
        IniAttributes.SaveToRegistry = True
        IniAttributes.FileName = 'PRODUKSI2'
        IniAttributes.Delimiter = ';;'
        ExportOptions.ExportType = wwgetSYLK
        ExportOptions.Options = [esoShowHeader, esoShowFooter, esoShowTitle, esoDblQuoteFields, esoBestColFit, esoShowRecordNo, esoShowAlternating]
        TitleColor = clBtnFace
        FixedCols = 0
        ShowHorzScrollBar = True
        Align = alClient
        Color = 16773589
        DataSource = dsQBrowse2
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgShowFooter, dgTrailingEllipsis, dgDblClickColSizing]
        ParentFont = False
        RowHeightPercent = 125
        TabOrder = 2
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlue
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = [fsBold]
        TitleLines = 3
        TitleButtons = True
        UseTFields = False
        OnTitleButtonClick = wwDBGrid2TitleButtonClick
        GroupFieldName = 'KONSTRUKSI'
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'Benang All'
      ImageIndex = 2
      OnShow = TabSheet3Show
      object Panel5: TPanel
        Left = 0
        Top = 0
        Width = 998
        Height = 52
        Align = alTop
        BevelInner = bvLowered
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 0
        object GroupBox3: TGroupBox
          Left = 2
          Top = 2
          Width = 287
          Height = 48
          Align = alLeft
          Caption = 'Filter Tanggal'
          TabOrder = 0
          TabStop = True
          object Label3: TLabel
            Left = 132
            Top = 22
            Width = 24
            Height = 13
            Caption = 'S/D'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object VTglAwal3: TwwDBDateTimePicker
            Left = 8
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 0
            DisplayFormat = 'dd mmm yyyy'
            OnChange = VTglAwal3Change
          end
          object vTglAkhir3: TwwDBDateTimePicker
            Left = 160
            Top = 16
            Width = 121
            Height = 24
            DisableThemes = True
            CalendarAttributes.Font.Charset = DEFAULT_CHARSET
            CalendarAttributes.Font.Color = clWindowText
            CalendarAttributes.Font.Height = -11
            CalendarAttributes.Font.Name = 'MS Sans Serif'
            CalendarAttributes.Font.Style = []
            Color = clGreen
            Epoch = 1950
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ShowButton = True
            TabOrder = 1
            DisplayFormat = 'dd mmm yyyy'
            OnChange = vTglAkhir3Change
          end
        end
        object Panel6: TPanel
          Left = 289
          Top = 2
          Width = 568
          Height = 48
          Align = alLeft
          BevelOuter = bvNone
          TabOrder = 1
          object vOperand3: TLabel
            Left = 150
            Top = 20
            Width = 44
            Height = 24
            Cursor = crHandPoint
            Alignment = taCenter
            AutoSize = False
            Caption = 'LIKE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            OnClick = vOperandClick
          end
          object cbTanggal3: TCheckBox
            Left = 200
            Top = 2
            Width = 129
            Height = 17
            Caption = 'Ikutkan Filter &Tanggal'
            Checked = True
            Enabled = False
            State = cbChecked
            TabOrder = 1
            OnClick = cbTanggalClick
          end
          object BitBtn6: TBitBtn
            Left = 328
            Top = 20
            Width = 75
            Height = 25
            Caption = 'Filte&r'
            Default = True
            ModalResult = 1
            TabOrder = 3
            OnClick = BitBtn6Click
            Glyph.Data = {
              
              5990555555555555577755555555555555555555555555555555}
            NumGlyphs = 2
          end
          object ECari3: TEdit
            Left = 200
            Top = 23
            Width = 121
            Height = 19
            Ctl3D = False
            ParentCtl3D = False
            TabOrder = 2
          end
          object cbOtomatis3: TCheckBox
            Left = 7
            Top = 5
            Width = 98
            Height = 17
            Caption = 'Cari &Otomatis'
            TabOrder = 0
            OnClick = cbOtomatisClick
          end
          object dbcField3: TwwDBComboBox
            Left = 8
            Top = 23
            Width = 137
            Height = 19
            ShowButton = True
            Style = csDropDown
            MapList = False
            AllowClearKey = False
            Color = clYellow
            DropDownCount = 8
            ItemHeight = 0
            Items.Strings = (
              'SEMUA KOLOM')
            ItemIndex = 0
            Sorted = False
            TabOrder = 4
            UnboundDataType = wwDefault
          end
          object BitBtn7: TBitBtn
            Left = 408
            Top = 20
            Width = 97
            Height = 25
            Caption = '&Export2Excel'
            TabOrder = 5
            OnClick = BitBtn7Click
            Glyph.Data = {
              
              0003737FFFFFFFFF7F7330099999999900333777777777777733}
            NumGlyphs = 2
          end
        end
      end
      object Panel8: TPanel
        Left = 0
        Top = 487
        Width = 998
        Height = 41
        Align = alBottom
        TabOrder = 1
        DesignSize = (
          998
          41)
        object Label2: TLabel
          Left = 16
          Top = 8
          Width = 5
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -19
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn8: TBitBtn
          Left = 742
          Top = 8
          Width = 75
          Height = 25
          Anchors = [akTop, akRight]
          TabOrder = 0
          Kind = bkClose
        end
      end
      object wwDBGrid3: TwwDBGrid
        Left = 0
        Top = 52
        Width = 998
        Height = 435
        DisableThemes = True
        DisableThemesInTitle = True
        ControlType.Strings = (
          'ISPOST;CheckBox;1;0'
          'PILIH;CheckBox;1;0')
        Selected.Strings = (
          'PILIH'#9'1'#9'PILIH'#9'F'
          'TGL'#9'18'#9'TANGGAL'#9'F'
          'KP'#9'20'#9'KP'#9'F'
          'NO_ORDER'#9'20'#9'NO RESEP'#9'F'
          'JML_POTONG'#9'10'#9'POTONG'#9'F'#9'ISI BEAM'
          'KD_KONSTRUKSI'#9'6'#9'KODE'#9'F'#9'KONSTRUKSI'
          'NAMA_KONSTRUKSI'#9'25'#9'NAMA'#9'F'#9'KONSTRUKSI'
          'KD_CORAK'#9'6'#9'KODE'#9'F'#9'CORAK'
          'NAMA_CORAK'#9'20'#9'NAMA'#9'F'#9'CORAK'
          'KD_ITEM'#9'15'#9'KODE'#9'F'#9'BAHAN'
          'NAMA_ITEM'#9'25'#9'NAMA'#9'F'#9'BAHAN'
          'RASIO'#9'10'#9'RASIO'#9'F'#9'BAHAN'
          'ARAH'#9'10'#9'ARAH'#9'F'
          'WARNA'#9'25'#9'WARNA'#9'F'
          'NHELAI'#9'10'#9'JML'#9'F'#9'HELAI'
          'THELAI'#9'10'#9'TOTAL'#9'F'#9'HELAI'
          'THELAI2'#9'10'#9'PAKAN'#9'F'#9'HELAI'
          'PHELAI'#9'10'#9'METER'#9'F'#9'PER HELAI'
          'KGHELAI'#9'10'#9'GRAM'#9'F'#9'PER HELAI'
          'PSARUNG'#9'10'#9'GRAM'#9'F'#9'PER PALEKAT'
          'KGSARUNG'#9'10'#9'KG'#9'F'#9'PER PALEKAT'
          'PKODI'#9'10'#9'METER'#9'F'#9'PER KODI'
          'KGKODI'#9'10'#9'KG'#9'F'#9'PER KODI'
          'KGWASTE'#9'10'#9'METER'#9'F'#9'WASTE KODI'
          'PWASTE'#9'10'#9'KG'#9'F'#9'WASTE KODI'
          'PNET'#9'10'#9'METER'#9'F'#9'NETTO KODI'
          'KGNET'#9'10'#9'KG'#9'F'#9'NETTO KODI')
        IniAttributes.Enabled = True
        IniAttributes.SaveToRegistry = True
        IniAttributes.FileName = 'PRODUKSI2'
        IniAttributes.Delimiter = ';;'
        ExportOptions.ExportType = wwgetSYLK
        ExportOptions.Options = [esoShowHeader, esoShowFooter, esoShowTitle, esoDblQuoteFields, esoBestColFit, esoShowRecordNo, esoShowAlternating]
        TitleColor = clBtnFace
        FixedCols = 0
        ShowHorzScrollBar = True
        Align = alClient
        Color = 16773589
        DataSource = dsQBrowse3
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgShowFooter, dgTrailingEllipsis, dgDblClickColSizing]
        ParentFont = False
        RowHeightPercent = 125
        TabOrder = 2
        TitleAlignment = taCenter
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlue
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = [fsBold]
        TitleLines = 3
        TitleButtons = True
        UseTFields = False
        OnTitleButtonClick = wwDBGrid2TitleButtonClick
        GroupFieldName = 'KONSTRUKSI'
      end
    end
  end
  object QBrowse: TOracleDataSet
    SQL.Strings = (
      'select a.*,a.rowid from ipisma_db4.vdesain_resep_all a'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D010000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      0000}
    QueryAllRecords = False
    CountAllRecords = True
    RefreshOptions = [roAfterInsert]
    Session = DMFrm.OS
    OnFilterRecord = QBrowseFilterRecord
    Left = 208
    Top = 152
    object QBrowseTGL: TDateTimeField
      FieldName = 'TGL'
    end
    object QBrowseNO_ORDER: TStringField
      DisplayWidth = 20
      FieldName = 'NO_ORDER'
      Size = 50
    end
    object QBrowseKD_KONSTRUKSI: TStringField
      DisplayWidth = 6
      FieldName = 'KD_KONSTRUKSI'
      Size = 50
    end
    object QBrowseNAMA_KONSTRUKSI: TStringField
      DisplayWidth = 50
      FieldName = 'NAMA_KONSTRUKSI'
      Size = 100
    end
    object QBrowseKD_CORAK: TStringField
      DisplayWidth = 6
      FieldName = 'KD_CORAK'
      Size = 10
    end
    object QBrowseNAMA_CORAK: TStringField
      FieldName = 'NAMA_CORAK'
      Size = 100
    end
    object QBrowseNO_DESAIN: TFloatField
      FieldName = 'NO_DESAIN'
    end
    object QBrowseARAH: TStringField
      DisplayWidth = 12
      FieldName = 'ARAH'
    end
    object QBrowseKELOMPOK: TStringField
      DisplayWidth = 12
      FieldName = 'KELOMPOK'
    end
    object QBrowseKD_ITEM: TStringField
      DisplayWidth = 50
      FieldName = 'KD_ITEM'
      Size = 50
    end
    object QBrowseNAMA_ITEM: TStringField
      DisplayWidth = 50
      FieldName = 'NAMA_ITEM'
      Size = 100
    end
    object QBrowseKD_WARNA: TStringField
      DisplayWidth = 6
      FieldName = 'KD_WARNA'
      Size = 10
    end
    object QBrowseWARNA: TStringField
      FieldName = 'WARNA'
      Size = 50
    end
    object QBrowseNO_BENANG: TFloatField
      FieldName = 'NO_BENANG'
    end
    object QBrowseRASIO: TFloatField
      FieldName = 'RASIO'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseNHELAI: TFloatField
      FieldName = 'NHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowsePSARUNG: TFloatField
      FieldName = 'PSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowsePKODI: TFloatField
      FieldName = 'PKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGSARUNG: TFloatField
      FieldName = 'KGSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGKODI: TFloatField
      FieldName = 'KGKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowsePWASTE: TFloatField
      FieldName = 'PWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGWASTE: TFloatField
      FieldName = 'KGWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowsePNET: TFloatField
      FieldName = 'PNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGNET: TFloatField
      FieldName = 'KGNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseTHELAI: TFloatField
      FieldName = 'THELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowsePHELAI: TFloatField
      FieldName = 'PHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGHELAI: TFloatField
      FieldName = 'KGHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseNO_URUT: TFloatField
      FieldName = 'NO_URUT'
    end
    object QBrowsePTOTAL: TFloatField
      FieldName = 'PTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKGTOTAL: TFloatField
      FieldName = 'KGTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseRASIO2: TFloatField
      FieldName = 'RASIO2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseTHELAI2: TFloatField
      FieldName = 'THELAI2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowseKD_KONV: TStringField
      FieldName = 'KD_KONV'
      Size = 5
    end
    object QBrowsePILIH: TStringField
      FieldName = 'PILIH'
      Required = True
      Size = 1
    end
  end
  object dsQBrowse: TwwDataSource
    DataSet = QBrowse
    Left = 536
    Top = 160
  end
  object QBrowseTotal: TOracleDataSet
    SQL.Strings = (
      
        'select sum(kghelai) as kghelai, sum(kgkodi) as kgkodi, sum(kgnet' +
        ')as kgnet,sum(kgsarung) as kgsarung, sum(kgtotal)as kgtotal,sum(' +
        'nhelai)as nhelai,'
      
        'sum(phelai)as phelai,sum(pkodi)as pkodi,sum(pnet) as pnet, sum(p' +
        'sarung) as psarung,'
      
        'sum(ptotal) as ptotal,sum(thelai)as thelai,sum(thelai2)as thelai' +
        '2  from ipisma_db4.vdesain_resep_all'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D010000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      414932010000000000070000004B47544F54414C010000000000}
    MasterFields = 'konstruksi'
    RefreshOptions = [roAfterInsert]
    Session = DMFrm.OS
    OnFilterRecord = QBrowseFilterRecord
    Left = 208
    Top = 256
    object QBrowseTotalKGHELAI: TFloatField
      FieldName = 'KGHELAI'
    end
    object QBrowseTotalKGKODI: TFloatField
      FieldName = 'KGKODI'
    end
    object QBrowseTotalKGNET: TFloatField
      FieldName = 'KGNET'
    end
    object QBrowseTotalKGSARUNG: TFloatField
      FieldName = 'KGSARUNG'
    end
    object QBrowseTotalKGTOTAL: TFloatField
      FieldName = 'KGTOTAL'
    end
    object QBrowseTotalNHELAI: TFloatField
      FieldName = 'NHELAI'
    end
    object QBrowseTotalPHELAI: TFloatField
      FieldName = 'PHELAI'
    end
    object QBrowseTotalPKODI: TFloatField
      FieldName = 'PKODI'
    end
    object QBrowseTotalPNET: TFloatField
      FieldName = 'PNET'
    end
    object QBrowseTotalPSARUNG: TFloatField
      FieldName = 'PSARUNG'
    end
    object QBrowseTotalPTOTAL: TFloatField
      FieldName = 'PTOTAL'
    end
    object QBrowseTotalTHELAI: TFloatField
      FieldName = 'THELAI'
    end
    object QBrowseTotalTHELAI2: TFloatField
      FieldName = 'THELAI2'
    end
  end
  object QBrowse2: TOracleDataSet
    SQL.Strings = (
      'select a.*,a.rowid from ipisma_db4.vdesain_resep_all a'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D010000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      50494C4948010000000000}
    MasterFields = 'konstruksi'
    RefreshOptions = [roAfterInsert]
    Session = DMFrm.OS
    OnFilterRecord = QBrowseFilterRecord
    Left = 296
    Top = 152
    object QBrowse2TGL: TDateTimeField
      FieldName = 'TGL'
    end
    object QBrowse2NO_ORDER: TStringField
      FieldName = 'NO_ORDER'
    end
    object QBrowse2KD_KONSTRUKSI: TStringField
      FieldName = 'KD_KONSTRUKSI'
      Size = 10
    end
    object QBrowse2NAMA_KONSTRUKSI: TStringField
      FieldName = 'NAMA_KONSTRUKSI'
      Required = True
      Size = 50
    end
    object QBrowse2KD_CORAK: TStringField
      FieldName = 'KD_CORAK'
      Size = 10
    end
    object QBrowse2NAMA_CORAK: TStringField
      FieldName = 'NAMA_CORAK'
      Required = True
      Size = 50
    end
    object QBrowse2NO_DESAIN: TFloatField
      FieldName = 'NO_DESAIN'
    end
    object QBrowse2ARAH: TStringField
      FieldName = 'ARAH'
      Size = 12
    end
    object QBrowse2KELOMPOK: TStringField
      FieldName = 'KELOMPOK'
      Size = 12
    end
    object QBrowse2KD_ITEM: TStringField
      FieldName = 'KD_ITEM'
      Size = 50
    end
    object QBrowse2NAMA_ITEM: TStringField
      FieldName = 'NAMA_ITEM'
      Size = 50
    end
    object QBrowse2KD_WARNA: TStringField
      FieldName = 'KD_WARNA'
      Size = 10
    end
    object QBrowse2WARNA: TStringField
      FieldName = 'WARNA'
      Size = 50
    end
    object QBrowse2NO_BENANG: TFloatField
      FieldName = 'NO_BENANG'
    end
    object QBrowse2RASIO: TFloatField
      FieldName = 'RASIO'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2NHELAI: TFloatField
      FieldName = 'NHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2PSARUNG: TFloatField
      FieldName = 'PSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2PKODI: TFloatField
      FieldName = 'PKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGSARUNG: TFloatField
      FieldName = 'KGSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGKODI: TFloatField
      FieldName = 'KGKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2PWASTE: TFloatField
      FieldName = 'PWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGWASTE: TFloatField
      FieldName = 'KGWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2PNET: TFloatField
      FieldName = 'PNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGNET: TFloatField
      FieldName = 'KGNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2THELAI: TFloatField
      FieldName = 'THELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2PHELAI: TFloatField
      FieldName = 'PHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGHELAI: TFloatField
      FieldName = 'KGHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2NO_URUT: TFloatField
      FieldName = 'NO_URUT'
    end
    object QBrowse2PTOTAL: TFloatField
      FieldName = 'PTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KGTOTAL: TFloatField
      FieldName = 'KGTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2RASIO2: TFloatField
      FieldName = 'RASIO2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2THELAI2: TFloatField
      FieldName = 'THELAI2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object QBrowse2KD_KONV: TStringField
      DisplayWidth = 25
      FieldName = 'KD_KONV'
      Size = 5
    end
    object QBrowse2PILIH: TStringField
      FieldName = 'PILIH'
      Size = 1
    end
  end
  object QBrowseTotal2: TOracleDataSet
    SQL.Strings = (
      
        'select sum(kghelai) as kghelai, sum(kgkodi) as kgkodi, sum(kgnet' +
        ')as kgnet,sum(kgsarung) as kgsarung, sum(kgtotal)as kgtotal,sum(' +
        'nhelai)as nhelai,'
      
        'sum(phelai)as phelai,sum(pkodi)as pkodi,sum(pnet) as pnet, sum(p' +
        'sarung) as psarung,'
      
        'sum(ptotal) as ptotal,sum(thelai)as thelai,sum(thelai2)as thelai' +
        '2  from ipisma_db4.vdesain_resep_all'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D010000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      414932010000000000070000004B47544F54414C010000000000}
    MasterFields = 'konstruksi'
    RefreshOptions = [roAfterInsert]
    Session = DMFrm.OS
    OnFilterRecord = QBrowseFilterRecord
    Left = 296
    Top = 256
    object QBrowseTotal2KGHELAI: TFloatField
      FieldName = 'KGHELAI'
    end
    object QBrowseTotal2KGKODI: TFloatField
      FieldName = 'KGKODI'
    end
    object QBrowseTotal2KGNET: TFloatField
      FieldName = 'KGNET'
    end
    object QBrowseTotal2KGSARUNG: TFloatField
      FieldName = 'KGSARUNG'
    end
    object QBrowseTotal2KGTOTAL: TFloatField
      FieldName = 'KGTOTAL'
    end
    object QBrowseTotal2NHELAI: TFloatField
      FieldName = 'NHELAI'
    end
    object QBrowseTotal2PHELAI: TFloatField
      FieldName = 'PHELAI'
    end
    object QBrowseTotal2PKODI: TFloatField
      FieldName = 'PKODI'
    end
    object QBrowseTotal2PNET: TFloatField
      FieldName = 'PNET'
    end
    object QBrowseTotal2PSARUNG: TFloatField
      FieldName = 'PSARUNG'
    end
    object QBrowseTotal2PTOTAL: TFloatField
      FieldName = 'PTOTAL'
    end
    object QBrowseTotal2THELAI: TFloatField
      FieldName = 'THELAI'
    end
    object QBrowseTotal2THELAI2: TFloatField
      FieldName = 'THELAI2'
    end
  end
  object dsQBrowse2: TwwDataSource
    DataSet = QBrowse2
    Left = 536
    Top = 240
  end
  object QBrowse3: TOracleDataSet
    SQL.Strings = (
      'select a.*,a.rowid from ipisma_db4.vdesain_resep_all a'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D01000000120000006F726465
      72206279204E4F5F4F524445520000000000}
    QBEDefinition.QBEFieldDefs = {
      
      4B500100000000000A0000004A4D4C5F504F544F4E47010000000000}
    MasterFields = 'konstruksi'
    RefreshOptions = [roAfterInsert]
    Session = DMFrm.OS
    OnFilterRecord = QBrowseFilterRecord
    Left = 384
    Top = 160
    object DateTimeField1: TDateTimeField
      FieldName = 'TGL'
    end
    object StringField1: TStringField
      FieldName = 'NO_ORDER'
    end
    object StringField2: TStringField
      FieldName = 'KD_KONSTRUKSI'
      Size = 6
    end
    object StringField3: TStringField
      FieldName = 'NAMA_KONSTRUKSI'
      Required = True
      Size = 50
    end
    object StringField4: TStringField
      FieldName = 'KD_CORAK'
      Size = 6
    end
    object StringField5: TStringField
      FieldName = 'NAMA_CORAK'
      Required = True
      Size = 50
    end
    object FloatField1: TFloatField
      FieldName = 'NO_DESAIN'
    end
    object StringField6: TStringField
      FieldName = 'ARAH'
      Size = 12
    end
    object StringField7: TStringField
      FieldName = 'KELOMPOK'
      Size = 12
    end
    object StringField8: TStringField
      FieldName = 'KD_ITEM'
      Size = 50
    end
    object StringField9: TStringField
      FieldName = 'NAMA_ITEM'
      Size = 50
    end
    object StringField10: TStringField
      FieldName = 'KD_WARNA'
      Size = 6
    end
    object StringField11: TStringField
      FieldName = 'WARNA'
      Size = 50
    end
    object FloatField2: TFloatField
      FieldName = 'NO_BENANG'
    end
    object FloatField3: TFloatField
      FieldName = 'RASIO'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField4: TFloatField
      FieldName = 'NHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField5: TFloatField
      FieldName = 'PSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField6: TFloatField
      FieldName = 'PKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField7: TFloatField
      FieldName = 'KGSARUNG'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField8: TFloatField
      FieldName = 'KGKODI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField9: TFloatField
      FieldName = 'PWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField10: TFloatField
      FieldName = 'KGWASTE'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField11: TFloatField
      FieldName = 'PNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField12: TFloatField
      FieldName = 'KGNET'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField13: TFloatField
      FieldName = 'THELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField14: TFloatField
      FieldName = 'PHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField15: TFloatField
      FieldName = 'KGHELAI'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField16: TFloatField
      FieldName = 'NO_URUT'
    end
    object FloatField17: TFloatField
      FieldName = 'PTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField18: TFloatField
      FieldName = 'KGTOTAL'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField19: TFloatField
      FieldName = 'RASIO2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object FloatField20: TFloatField
      FieldName = 'THELAI2'
      DisplayFormat = '#,##0.##;(#,##0.##) '
    end
    object StringField12: TStringField
      FieldName = 'KD_KONV'
      Size = 5
    end
    object QBrowse3PILIH: TStringField
      FieldName = 'PILIH'
      Size = 1
    end
    object QBrowse3KP: TStringField
      FieldName = 'KP'
    end
    object QBrowse3JML_POTONG: TFloatField
      FieldName = 'JML_POTONG'
    end
  end
  object dsQBrowse3: TwwDataSource
    DataSet = QBrowse3
    Left = 544
    Top = 328
  end
  object QAmbil_Data: TOracleQuery
    SQL.Strings = (
      'begin'
      '  ipisma_db4.Proc_rekap_desain_lusi(:pawal,:pakhir);'
      'end;')
    Session = DMFrm.OS
    Variables.Data = {
      0300000002000000060000003A504157414C0C0000000700000078710B010101
      0100000000070000003A50414B4849520C0000000700000078710B0B01010100
      000000}
    Left = 208
    Top = 208
  end
  object QAmbil_Data2: TOracleQuery
    SQL.Strings = (
      'begin'
      '  ipisma_db4.Proc_rekap_desain2(:pawal,:pakhir);'
      'end;')
    Session = DMFrm.OS
    Variables.Data = {
      0300000002000000060000003A504157414C0C0000000700000078710B010101
      0100000000070000003A50414B4849520C0000000700000078710B0B01010100
      000000}
    Left = 296
    Top = 208
  end
  object QAmbil_Data3: TOracleQuery
    SQL.Strings = (
      'begin'
      '  ipisma_db4.Proc_rekap_desain(:pawal,:pakhir);'
      'end;')
    Session = DMFrm.OS
    Variables.Data = {
      0300000002000000060000003A504157414C0C0000000700000078710B010101
      0100000000070000003A50414B4849520C0000000700000078710B0B01010100
      000000}
    Left = 384
    Top = 216
  end
  object QBrowseTotal3: TOracleDataSet
    SQL.Strings = (
      
        'select sum(kghelai) as kghelai, sum(kgkodi) as kgkodi, sum(kgnet' +
        ')as kgnet,sum(kgsarung) as kgsarung, sum(kgtotal)as kgtotal,sum(' +
        'nhelai)as nhelai,'
      
        'sum(phelai)as phelai,sum(pkodi)as pkodi,sum(pnet) as pnet, sum(p' +
        'sarung) as psarung,'
      
        'sum(ptotal) as ptotal,sum(thelai)as thelai,sum(thelai2)as thelai' +
        '2  from ipisma_db4.vdesain_resep_all'
      ':myparam')
    Variables.Data = {
      0300000001000000080000003A4D59504152414D010000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      
      4C4149010000000000070000005448454C414932010000000000}
    Session = DMFrm.OS
    Left = 376
    Top = 272
    object QBrowseTotal3KGHELAI: TFloatField
      FieldName = 'KGHELAI'
    end
    object QBrowseTotal3KGKODI: TFloatField
      FieldName = 'KGKODI'
    end
    object QBrowseTotal3KGNET: TFloatField
      FieldName = 'KGNET'
    end
    object QBrowseTotal3KGSARUNG: TFloatField
      FieldName = 'KGSARUNG'
    end
    object QBrowseTotal3KGTOTAL: TFloatField
      FieldName = 'KGTOTAL'
    end
    object QBrowseTotal3NHELAI: TFloatField
      FieldName = 'NHELAI'
    end
    object QBrowseTotal3PHELAI: TFloatField
      FieldName = 'PHELAI'
    end
    object QBrowseTotal3PKODI: TFloatField
      FieldName = 'PKODI'
    end
    object QBrowseTotal3PNET: TFloatField
      FieldName = 'PNET'
    end
    object QBrowseTotal3PSARUNG: TFloatField
      FieldName = 'PSARUNG'
    end
    object QBrowseTotal3PTOTAL: TFloatField
      FieldName = 'PTOTAL'
    end
    object QBrowseTotal3THELAI: TFloatField
      FieldName = 'THELAI'
    end
    object QBrowseTotal3THELAI2: TFloatField
      FieldName = 'THELAI2'
    end
  end
end
