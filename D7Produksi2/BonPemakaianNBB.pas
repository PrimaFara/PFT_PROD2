...
type
  TBonPemakaianNBBFrm = class(TForm)
    Panel1: TPanel;
...
var
  BonPemakaianNBBFrm: TBonPemakaianNBBFrm;
...
procedure TBonPemakaianNBBFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  BonPemakaianNBBFrm:=Nil;
end;

procedure TBonPemakaianNBBFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QLookSubBagian.Close;
  QLookItem.Close;
end;

procedure TBonPemakaianNBBFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TBonPemakaianNBBFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TBonPemakaianNBBFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TBonPemakaianNBBFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TBonPemakaianNBBFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
  QLookItem.Close;
...
procedure TBonPemakaianNBBFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TBonPemakaianNBBFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TBonPemakaianNBBFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TBonPemakaianNBBFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBonPemakaianNBBFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TBonPemakaianNBBFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TBonPemakaianNBBFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TBonPemakaianNBBFrm.LookBagianCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
    QDetailID_SUB_BAG.AsString:=QLookSubBagianID_SUB_BAG.AsString;
...
procedure TBonPemakaianNBBFrm.LookItemEnter(Sender: TObject);
begin
  QLookItem.Open;
end;

procedure TBonPemakaianNBBFrm.QLookItemBeforeOpen(DataSet: TDataSet);
var
  vdd,vmm,vyy : word;
  vtgl_awal : TDate;
begin
...
procedure TBonPemakaianNBBFrm.LookBagianEnter(Sender: TObject);
begin
  QLookSubBagian.Open;
end;

procedure TBonPemakaianNBBFrm.LookItemCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TBonPemakaianNBBFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TBonPemakaianNBBFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TBonPemakaianNBBFrm.ECariChange(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TBonPemakaianNBBFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TBonPemakaianNBBFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TBonPemakaianNBBFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TBonPemakaianNBBFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TBonPemakaianNBBFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.QLookSubBagianBeforeOpen(DataSet: TDataSet);
begin
  QLookSubBagian.SetVariable('kd_bag',vkd_bag);
end;

procedure TBonPemakaianNBBFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TBonPemakaianNBBFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
...
procedure TBonPemakaianNBBFrm.FormCloseQuery(Sender: TObject;
...
procedure TBonPemakaianNBBFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBonPemakaianNBBFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TBonPemakaianNBBFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TBonPemakaianNBBFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TBonPemakaianNBBFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TBonPemakaianNBBFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TBonPemakaianNBBFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TBonPemakaianNBBFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TBonPemakaianNBBFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TBonPemakaianNBBFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TBonPemakaianNBBFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TBonPemakaianNBBFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TBonPemakaianNBBFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TBonPemakaianNBBFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TBonPemakaianNBBFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TBonPemakaianNBBFrm.LookItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TBonPemakaianNBBFrm.LookBagianUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TBonPemakaianNBBFrm.LookItemUserButton2Click(Sender: TObject;
  LookupTable: TDataSet);
begin
...
procedure TBonPemakaianNBBFrm.QDetailBeforePost(DataSet: TDataSet);
begin
